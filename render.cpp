...
int initGL(void)
{
    ...
    {
     ver_triangle[i]=-1.0f;  //x dla pierwszego koła
     ver_triangle[i+3]=1.0f; //x dla drugiego koła

     ver_triangle[i+1]=r*sin(2*M_PI*i/100); //dla y z pierwszego koła
     ver_triangle[i+4]=r*sin(2*M_PI*i/100); //dla y z drugiego koła

     ver_triangle[i+2]=r*cos(2*M_PI*i/100); //z dla pierwszego kołą
     ver_triangle[i+5]=r*cos(2*M_PI*i/100); //z dla drugiego koła

     col_triangle[i]=1.0f;
     col_triangle[i+1]=0.0f;
     col_triangle[i+2]=0.0f;
     col_triangle[i+3]=0.0f;
     col_triangle[i+4]=1.0f;
     col_triangle[i+5]=0.0f;
    }
    if(initShaders())
    {   
...
int drawGLScene(int counter)
{
  ...
  x=radius;
   for(int i=0;i<count;i++){ 
    ...
    glm::mat4 translationMatrix = glm::translate(glm::mat4(), glm::vec3(-1.4f+step*i, distance, 0.0f));  		//macierz przesuniecia o zadany wektor
    ...
    glm::mat4 scaleMatrix=glm::scale(glm::mat4(), glm::vec3(scale, scale, scale));
    glm::mat4 transformMatrix = projectionMatrix * viewMatrix * translationMatrix *scaleMatrix; //wygenerowanie macierzy uwzgledniajacej wszystkie transformacje
  ...
  { 
    ...
    glm::mat4 translationMatrix = glm::translate(glm::mat4(), glm::vec3(-1.4f+step*i, distance+up, 0.0f));  		//macierz przesuniecia o zadany wektor
    //glm::mat4 rotationMatrix = glm::rotate(glm::mat4(), glm::radians(fi), glm::vec3(0.0f, 1.0f, 0.0f)); //macierz obrotu o dany kat wokol wektora
 ...
 for(int i=0;i<count;i++){ 
    ...
    glm::mat4 translationMatrix = glm::translate(glm::mat4(), glm::vec3(-1.4f+step*i, distance+up*2, 0.0f));  		//macierz przesuniecia o zadany wektor
    //glm::mat4 rotationMatrix = glm::rotate(glm::mat4(), glm::radians(fi), glm::vec3(0.0f, 1.0f, 0.0f)); //macierz obrotu o dany kat wokol wektora
...
for(int i=0;i<count-1;i++){ 
    ...
    glm::mat4 translationMatrix = glm::translate(glm::mat4(), glm::vec3(-0.7f+step*1.8*i, distance-0.15f, -0.1f));  		//macierz przesuniecia o zadany wektor
    glm::mat4 rotationMatrix = glm::rotate(glm::mat4(), glm::radians(fi), glm::vec3(1.0f, 0.0f, 0.0f)); //macierz obrotu o dany kat wokol wektora
    ...
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 200); //rysowanie trojkata
   ...
   { 
    ...
    glm::mat4 translationMatrix = glm::translate(glm::mat4(), glm::vec3(-0.7f+step*1.8*i, distance-0.15f+up, -0.1f));  		//macierz przesuniecia o zadany wektor
    glm::mat4 rotationMatrix = glm::rotate(glm::mat4(), glm::radians(fi), glm::vec3(1.0f, 0.0f, 0.0f)); //macierz obrotu o dany kat wokol wektora
   ...
   for(int i=0;i<count-1;i++){ 
    ...
    glm::mat4 translationMatrix = glm::translate(glm::mat4(), glm::vec3(-0.7f+step*1.8*i, distance-0.15f+2*up, -0.1f));  		//macierz przesuniecia o zadany wektor
    glm::mat4 rotationMatrix = glm::rotate(glm::mat4(), glm::radians(fi), glm::vec3(1.0f, 0.0f, 0.0f)); //macierz obrotu o dany kat wokol wektora
   ...
   for(int i=0;i<count;i++){ 
    ...
    glm::mat4 translationMatrix = glm::translate(glm::mat4(), glm::vec3(-1.4f+step*i, distance+2*up, -0.1f));  		//macierz przesuniecia o zadany wektor
    glm::mat4 rotationMatrix = glm::rotate(glm::mat4(), glm::radians(fi+90), glm::vec3(0.0f, 0.0f, 1.0f)); //macierz obrotu o dany kat wokol wektora
   ...
   for(int i=0;i<count;i++){ 
    ...
    glm::mat4 translationMatrix = glm::translate(glm::mat4(), glm::vec3(-1.4f+step*i, distance+up*0.6f, -0.1f));  		//macierz przesuniecia o zadany wektor
    glm::mat4 rotationMatrix = glm::rotate(glm::mat4(), glm::radians(fi+90), glm::vec3(0.0f, 0.0f, 1.0f)); //macierz obrotu o dany kat wokol wektora
